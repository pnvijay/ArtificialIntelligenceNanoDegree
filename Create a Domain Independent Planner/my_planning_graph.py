...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # done on the basis of udacity discussions and slack channel discussions
        # first define set of a_levels of self
        self.a_levels.append(set())
        # loop through all actions of self
        # create new PgNode_a
        # check if they are subsets of s_levels in that level
        # then add the PgNode_a to a_levels
        # create parent and child bindings to s_levels if s_levels are part of prenodes of PgNode_a
        for action in self.all_actions:
            new_action_nodes = PgNode_a(action)
            if new_action_nodes.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(new_action_nodes)
                for s in self.s_levels[level]:
                    if s in new_action_nodes.prenodes:
                        s.children.add(new_action_nodes)
                        new_action_nodes.parents.add(s)
    ...
    def add_literal_level(self, level):
        ...
        # loop through a_levels of self in previous level
        # loop through eff nodes of the a_level
        # add the eff nodes to the s_levels
        # do the parent and children linkages
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # inefficient code given in one of udacity discussions was used as basis to start
        # create action instances of the two nodes
        # check if effect_add of the first instance is the same as effect_rem of the second node action instance
        # check if effect_rem of the first instance is the same as effect_add of the second node action instance
        # return true if any of the above is matching
        # return false otherwise

        a1 = node_a1.action
        a2 = node_a2.action
        for e in a1.effect_add:
            for f in a2.effect_rem:
                if e == f:
                    return True
        for e in a1.effect_rem:
            for f in a2.effect_add:
                if e == f:
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # create action instances of the two nodes
        # check if effect_add of the first instance is the same as
        # postive and negative precondiitons of the second node action instance
        # check if effect_rem of the first instance is the same as
        # postive and negative precondiitons of the second node action instance
        # return true if any of the above is matching
        # return false otherwise
        ...
        for e in a1.effect_add:
            for f in a2.precond_pos:
                if e == f:
                    return True
        for e in a1.effect_add:
            for f in a2.precond_neg:
                if e == f:
                    return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # create action instances of the two nodes
        # check if positive preconditions of the first instance is the same as
        # negative precondiitons of the second node action instance
        # check if negative preconditions of the first instance is the same as
        # postive and negative precondiitons of the second node action instance
        # do the same by reversing the first and second instances
        # Also invoke parents for the two nodes and check if they are mutex of each other
        # return true if any of the above is matching
        # return false otherwise
        ...
        for e in a1.precond_pos:
            for f in a2.precond_neg:
                if e == f:
                    return True

        for e in a1.precond_neg:
            for f in a2.precond_pos:
                if e == f:
                    return True

        for e in a2.precond_neg:
            for f in a1.precond_pos:
                if e == f:
                    return True

        for e in a2.precond_pos:
            for f in a1.precond_neg:
                if e == f:
                    return True
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # Since the _eq_ itself is not used, we the self.symbol property of this literal to check
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # Done on basis of udacity discussions. Special Thanks to Leticia for her posts
        # Invoke the parents of the nodes
        # check if they are mutex of each other
        # return global True but return False only if NOT p1 is mutex of p2 and NOT p2 is mutex of p2 is True
    ...
    def h_levelsum(self) -> int:
        ...
        # done on basis of udacity discussions and slack channel discussions
        # loop through goals of the problem
        # Take the s_levels and check at what levels the goal state is reached
        # add the levels together to get the level costs
